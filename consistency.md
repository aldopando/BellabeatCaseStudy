...
**Even though we observed inconsistent totals in the calories tables, we now know this was due to rounding decimals in the minute table. For the other variables, we didn’t find any inconsistencies in either the time period or the values, especially since those tables use integer values. Therefore, knowing that the minute and hourly tables are consistent with each other in the first dataset, we will use only the hourly tables for our analysis and to build our daily table**.
